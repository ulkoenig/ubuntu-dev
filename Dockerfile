...
ARG ARG_OPENSHIFTTOOLS=TRUE
ARG ARG_ECLIPSE=FALSE
ARG ARG_NETBEANS=FALSE
ARG ARG_INTELLIJ=FALSE
ARG ARG_POSTMAN=TRUE
ARG ARG_DOCKER=TRUE
ARG ARG_NODEJS=TRUE
ARG ARG_GEDIT=FALSE
ARG ARG_PYTHON=TRUE    

ENV INSTALL_OPENSHIFTTOOLS=${ARG_OPENSHIFTTOOLS} \
    INSTALL_ECLIPSE=${ARG_ECLIPSE} \
    INSTALL_NETBEANS=${ARG_NETBEANS} \
    INSTALL_INTELLIJ=${ARG_INTELLIJ} \
    INSTALL_POSTMAN=${ARG_POSTMAN} \
    INSTALL_DOCKER=${ARG_DOCKER} \
    INSTALL_NODEJS=${ARG_NODEJS} \
    INSTALL_GEDIT=${ARG_GEDIT} \
    INSTALL_PYTHON=${ARG_PYTHON}
...
RUN $INST_SCRIPTS/tigervnc.sh && \
    ...
    $INST_SCRIPTS/python.sh -i $INSTALL_PYTHON && \
